...
import electronLog, { LogLevel as LogLevel, transports } from 'electron-log';
import * as fs from 'fs';
import * as path from 'path';
import * as util from 'util';
import { isElectronQA, isLinux, isWindowsOS } from './env.helper';
import { getCommandLineArgs } from './utils.helper';
...
class Logger {
  ...
  constructor() {
    ...
    if (app.isPackaged) {
      ...
      transports.file.format =
        'Infinity {y}-{m}-{d} {h}:{i}:{s}:{ms} {z} | {level} | {text}';
  ...
  private log(logLevel: LogLevel, message: string, data: any[] = []): void {
    ...
    if (!isElectronQA) {
      switch (logLevel) {
        case 'error':
          electronLog.error(message, 'color: red');
          break;
        case 'warn':
